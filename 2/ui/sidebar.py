...
from utils.converters import html_to_svg, svg_to_html
        ...
        try:
            ...
            if data is not None:
                if selected_option == 'svg':
                    converted_content = html_to_svg(data, selected_option)
                elif selected_option == 'html':
                    converted_content = svg_to_html(data, selected_option)
                else:
                    raise "Неверный формат для конвертации"
                self.main_frame.edit_box.delete("1.0", "end-1c")
